...
function [stks_cyl] = geometry_cylinderPair(rho2,dsep2,psi2,PRAx2,PRAy2,mod1)
    ...
    circle = [sin(theta);cos(theta);zeros(floor(rho2*2*pi),1)']'; % This is the co-ords of the surface of the appendage.
    circleR = [sin(-theta);cos(-theta);zeros(floor(rho2*2*pi),1)']; 
    ...
    stks_cyl2 = circleR + [PRAx2-(1+dsep2/2)*cos(psi2),PRAy2-(1+dsep2/2)*sin(psi2),5+2*mod1]; % Shift the second circle to + side of the separation.
