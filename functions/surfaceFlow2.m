function [Uflow] = surfaceFlow2(nTemp1,rot1)
...
Uflow(:,1) = surfF*sign(magnitude).*sqrt(abs(magnitude)).*tangent(:,1);
Uflow(:,2) = surfF*sign(magnitude).*sqrt(abs(magnitude)).*tangent(:,2);
